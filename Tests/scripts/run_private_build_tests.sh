...
python3 ./Tests/private_build/configure_and_test_integration_instances_private.py -u "$USERNAME" -p "$PASSWORD" -c "$CONF_PATH" -s "$SECRET_CONF_PATH" -g "$GIT_SHA1" --ami_env "$1" -n $IS_NIGHTLY --branch "$BRANCH_NAME" --build-number "$GITHUB_RUN_NUMBER" -pr true --test_pack_path '/home/runner/work/content-private/content-private/content/temp_artifacts/packs'
code_1=$?
