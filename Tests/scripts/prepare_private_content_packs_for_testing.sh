...
ARTIFACTS=${ARTIFACTS}
PACK_ARTIFACTS=$ARTIFACTS/content_packs.zip
ID_SET=$ARTIFACTS/id_set.json
...
if [ ! -f $CONTENT_PACKS_TO_INSTALL_FILE ]; then
  ...
  if [[ -z "$CONTENT_PACKS_TO_INSTALL" ]]; then
    ...
    python3 ./Tests/private_build/upload_packs_private.py -b $GCS_TESTING_BUCKET -pb $GCS_PRIVATE_TESTING_BUCKET -a $PACK_ARTIFACTS -d $ARTIFACTS/packs_dependencies.json -e $EXTRACT_FOLDER -s $KF -n $GITHUB_RUN_NUMBER -p $NEW_PACK_NAME -sb $PUBLIC_TARGET_PATH -k $PACK_SIGN_KEY -rt false --id_set_path $ID_SET -pr True -bn $GIT_BRANCH -ek $PACK_ENCRYPTION_KEY -o
